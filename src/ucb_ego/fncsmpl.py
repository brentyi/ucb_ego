"""Somewhat opinionated wrapper for the SMPL body model.

We break down the SMPL into four stages, each with a corresponding data structure:
- Loading the SMPL model itself:
    `model = SmplModel.load(path to npz)`
- Applying a body shape to the SMPL model:
...
class SmplModel(TensorDataclass):
    """A human body model from the SMPL family."""

    faces: Int[Tensor, "13776 3"]
    ...
    """LBS weights."""
    posedirs: Float[Tensor, "verts 3 459"]
    """Pose blend shape bases."""
    v_template: Float[Tensor, "verts 3"]
    """Canonical mesh verts."""
    shapedirs: Float[Tensor, "verts 3 16"]
    """Shape bases."""

    @staticmethod
    def load(model_path: Path) -> SmplModel:
    ...
    def with_shape(self, betas: Float[Tensor, "*#batch 16"]) -> SmplShaped:
...
@jaxtyped(typechecker=typeguard.typechecked)
@dataclass(frozen=True)
class SmplShaped(TensorDataclass):
...
@jaxtyped(typechecker=typeguard.typechecked)
@dataclass(frozen=True)
class SmplShapedAndPosed(TensorDataclass):
